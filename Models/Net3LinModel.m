classdef Net3LinModel < NetModels
    % Species:
    %   AA - active A
    %   BA - active B
    %   CA - active C
    % Reaction network (deterministic):
%     z[0] =  c1 * u * (AT-A)  -  c2 * A * B 
%     z[1] =  c3 * A * (BT-B)  -  c4 * B
%     z[2] =  c5 * A * (CT-C)  -  c6 * B * C  -  c7 * C
    ...
    %
    % METHODS:
    %   model = Net3LinModel(params, input, deltaInput) ALL OPT INPUTS
    ...
    methods
        function model = Net3LinModel(params, input, deltaInput)
